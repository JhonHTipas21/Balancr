...
class BankCSVSource(TransactionSource):
    """
    Adapter to parse bank statement transaction data from a CSV file.
    """
    ...
    def load_transactions(self) -> List[CanonicalTransaction]:
        ...
        try:
            ...
            raise ValueError(f"Failed to read Bank CSV file at {self.file_path}: {e}")

        # Check required columns
        for canonical_key, csv_header in self.col_mapping.items():
            if csv_header not in df.columns:
                raise ValueError(f"Required header '{csv_header}' for canonical field '{canonical_key}' not found in Bank CSV.")
        ...
        for _, row in df.iterrows():
            ...
            if isinstance(raw_date, datetime):
                ...
                try:
                    ...
                    raise ValueError(f"Error parsing bank date '{raw_date}' in row {tx_id}: {e}")

...
class GatewayCSVSource(TransactionSource):
    """
    Adapter to parse gateway transaction data (e.g. Wompi, Stripe) from a CSV file.
    """
    ...
    def load_transactions(self) -> List[CanonicalTransaction]:
        ...
        try:
            ...
            raise ValueError(f"Failed to read Gateway CSV file at {self.file_path}: {e}")
        ...
        for canonical_key, csv_header in self.col_mapping.items():
            if csv_header not in df.columns:
                raise ValueError(f"Required header '{csv_header}' for canonical field '{canonical_key}' not found in Gateway CSV.")

        for _, row in df.iterrows():
            # Convert row to dict for raw_payload
            raw_data = row.to_dict()
            
            # Extract and parse values
            tx_id = str(row[self.col_mapping["id"]])
            
            # Amount handling: strip symbols and convert to float
            ...
            if isinstance(raw_date, datetime):
                ...
                try:
                    ...
                    raise ValueError(f"Error parsing date '{raw_date}' in row {tx_id}: {e}")
